...
<body>
    ...
    <header>
        <div class="barra"></div> 
        <h2 class="titulo"><b>Gestión de aulas</b></h2>
        <div class="barra"></div>
    </header>
    ...
    $result = $mysqli->query("SELECT idSalon, identificador, piso, categoria, capacidadUso, climas, horaApertura, horaCierre FROM salon");
    ...
    while ($row = $result->fetch_assoc()) {
        echo '<tr>
                ...
                <td>' . $row['piso'] . '</td>
                <td>' . $row['categoria'] . '</td>
                <td>' . $row['capacidadUso'] . '</td>
                <td>' . ($row['climas'] ? 'Sí' : 'No') . '</td>
                <td>' . $row['horaApertura'] . '</td>
                <td>' . $row['horaCierre'] . '</td>
                <td>
                    <button type="button" data-id="' . $row['idSalon'] . '" class="btn btn-sm btn-danger eliminar"><i class="fa-solid fa-trash-can"></i></button>
                    <button type="button" data-id="' . $row['idSalon'] . '" class="btn btn-sm btn-primary edit-button"><i class="fa-solid fa-pencil"></i></button>
                </td>
            </tr>';
    }
    ...
    <script>
        ...
        $(document).on('click', '.eliminar', function () {
            ...
            var id = $(this).data('id'); // Obtener el ID del salón del atributo de datos
            Swal.fire({
                title: '¿Estás seguro?',
                text: "Vas a eliminar el salón con id " + id,
                icon: 'warning',
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if(response.status == "success"){
                                ...
                                Swal.fire(
                                    'Eliminado!',
                                    'El salón ha sido eliminado exitosamente.',
                                    'success'
                                )
                            } else {
                                Swal.fire(
                                    'Error!',
                                    'Hubo un error al eliminar el salón.',
                                    'error'
                                )
                            }
                        },
                        error: function(jqXHR, textStatus, errorThrown) {
                            if (textStatus === 'parsererror') {
                                Swal.fire(
                                    'Error!',
                                    'Hubo un error al analizar la respuesta del servidor.',
                                    'error'
                                )
                            } else {
                                Swal.fire(
